/* ============================================================
   LATITUDE SAMUI — Modal de contact partagé / Intentions v2
   ...
   Configurez window.LATITUDE_WEB3FORMS_KEY avant ce script.
   ============================================================ */
(function () {
  'use strict';
  ...
  var css = `
    ...
    .modal{width:100%;max-width:700px;max-height:min(92vh,840px);display:flex;flex-direction:column;position:relative;overflow:hidden;background:var(--white,#faf7f0);color:var(--ink,#1a1e1a);box-shadow:0 30px 100px rgba(0,0,0,.38);animation:contactModalIn .35s cubic-bezier(.22,1,.36,1)}
    .modal-topbar{min-height:64px;display:flex;align-items:center;justify-content:space-between;gap:1rem;padding:.8rem 1rem .75rem 2.5rem;border-bottom:1px solid rgba(26,30,26,.08);background:var(--white,#faf7f0);flex:0 0 auto}
    .modal-topbar-copy{display:grid;gap:.18rem;min-width:0}
    .modal-context{font-family:Inter,sans-serif;font-size:.58rem;letter-spacing:.22em;text-transform:uppercase;color:var(--terracotta,#b8623f);white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
    .modal-step-label{font-family:Inter,sans-serif;font-size:.61rem;letter-spacing:.18em;text-transform:uppercase;color:var(--gold-deep,#9c7b45)}
    .modal-close{width:40px;height:40px;display:flex;align-items:center;justify-content:center;background:transparent;border:1px solid rgba(26,30,26,.15);border-radius:50%;cursor:pointer;color:var(--ink,#1a1e1a);transition:all .2s;flex:0 0 auto}
    ...
    .modal-step h3{font-family:"Cormorant Garamond",serif;font-size:clamp(1.8rem,4vw,2.4rem);line-height:1.05;font-weight:500;margin:0 3rem .65rem 0}
    ...
    .stage-question{display:none;margin:1rem 0 .7rem;font-family:Inter,sans-serif;font-size:.62rem;letter-spacing:.19em;text-transform:uppercase;color:var(--gold-deep,#9c7b45)}
    ...
    .req-note{font-family:Inter,sans-serif;font-size:.66rem;line-height:1.55;color:var(--ink-soft,#4a4e4a);margin-top:.7rem}
    .villa-context-card{display:none;grid-template-columns:minmax(220px,40%) minmax(0,1fr);align-items:center;gap:1rem;margin:0 0 1.15rem;padding:.75rem;border:1px solid rgba(201,160,88,.42);background:rgba(201,160,88,.07);overflow:hidden}
    .villa-context-card.show{display:grid}
    .villa-context-media{width:100%;aspect-ratio:16/9;overflow:hidden;background:var(--sand-light,#f2ede2)}
    ...
    .villa-context-kicker{font-family:Inter,sans-serif;font-size:.54rem;letter-spacing:.19em;text-transform:uppercase;color:var(--gold-deep,#9c7b45);margin-bottom:.35rem}
    .villa-context-name{font-family:"Cormorant Garamond",serif;font-size:1.45rem;font-weight:600;line-height:1.05;margin-bottom:.35rem}
    .villa-context-location{font-family:Inter,sans-serif;font-size:.7rem;letter-spacing:.06em;color:var(--ink-soft,#4a4e4a)}
    .villa-context-choices{display:none;grid-template-columns:auto minmax(0,1fr);gap:.3rem .8rem;margin-top:.65rem;font-family:Inter,sans-serif;font-size:.76rem;line-height:1.4}
    .villa-context-choices.show{display:grid}
    .villa-context-choices span{color:var(--ink-soft,#4a4e4a)}
    .villa-context-choices strong{font-weight:600;color:var(--terracotta,#b8623f)}
    .message-section{margin-top:1.1rem}
    .message-label{font-family:Inter,sans-serif;font-size:.62rem;letter-spacing:.18em;text-transform:uppercase;color:var(--gold-deep,#9c7b45);margin-bottom:.65rem}
    ...
    .check-icon{width:72px;height:72px;margin:0 auto 1.25rem;border-radius:50%;display:flex;align-items:center;justify-content:center;color:var(--white,#faf7f0);background:var(--success,#1a2e22)}
    .check-icon svg{width:34px;height:34px}
    .modal-step.success p{font-family:Inter,sans-serif;font-size:.9rem;line-height:1.65;color:var(--ink-soft,#4a4e4a);margin-top:1rem}
    ...
    @media(max-width:640px){
      ...
      .modal-context{font-size:.53rem}
      .modal-step-label{font-size:.56rem}
        ...
        <div class="modal-content" id="modalContent">
          ...
          <div class="modal-step" id="stepContact" data-step>
            ...
            <div class="form-group">
              <div class="form-row">
                <input type="text" class="form-input" id="contactFirstName" placeholder="Prénom *" autocomplete="given-name">
                <input type="text" class="form-input" id="contactLastName" placeholder="Nom *" autocomplete="family-name">
              </div>
              __COUNTRY_SELECT__
              <p class="form-error" id="err-identity"></p>
              <div class="phone-row">
                <span class="phone-prefix" id="phonePrefix">+…</span>
                <input type="tel" class="form-input" id="contactPhone" placeholder="Téléphone" autocomplete="tel-national" inputmode="tel">
              </div>
  ...
  function getContextKey(ctx) {
    return [ctx.intent, normalizedVilla(ctx.villa) || 'general'].join('|');
  }
  ...
  function isConfigured() {
    return WEB3FORMS_KEY && WEB3FORMS_KEY.indexOf('VOTRE_CLE') === -1;
  }
  ...
  function submitContact() {
    if (!isConfigured()) {
      status.textContent = 'Le formulaire doit encore être connecté à Web3Forms avant la mise en ligne.';
      return;
    }
    ...
    if (currentIntent === 'estimate' && currentContext.configuration) {
      payload['Approche intérieure choisie'] = currentContext.configuration.interior || 'Carte Blanche';
      payload['Approche jardin choisie'] = currentContext.configuration.garden || 'Carte Blanche';
      payload['Configuration modifiée par le visiteur'] = currentContext.configuration.touched ? 'Oui' : 'Non — configuration incluse par défaut';
    }
    ...
    fetch('https://api.web3forms.com/submit', {
      ...
      .then(function (result) {
        if (!result.ok || result.data.success === false) {
          throw new Error(result.data.message || 'Envoi impossible');
        }
        showSuccess();
      })
      .catch(function () {
