(function(root){
  ...
  const INTERIORS=['Essentiel','Éveil des sens','Art de vivre'];
  const GARDENS=['Essentiel','Horizon','Art de vivre'];
  function buildLead(data,context,configuration,language){
    ...
    const lead={intent,language:language==='en'?'en':'fr',name:clean(data.name,120),email:clean(data.email,200),phone:clean(data.phone,40),message:clean(data.message),stage:clean(data.stage,80),visit:intent==='visit'?clean(data.visit,80):'',villa:clean(context.villa,100),source:clean(context.source,100),website:clean(data.website,200)};
    if(configuration&&(!lead.villa||lead.villa===configuration.villa)){
      lead.villa=clean(configuration.villa,100);lead.configuration={interior:INTERIORS.includes(configuration.interior)?configuration.interior:'Essentiel',garden:GARDENS.includes(configuration.garden)?configuration.garden:'Essentiel'};
    }
    return lead;
  }
