...
(() => {
  ...
  const theme=`
    .o-livechat-LivechatButton,.o_livechat_button{
      ...
      color:#fff!important;background:#102c25!important;border:1px solid #ffffff55!important;
    ...
    .o-livechat-LivechatButton::after,.o_livechat_button::after{
      content:'${document.documentElement.lang==='en'?'Chat':'Discuter'}';font:500 14px/1.2 Manrope,Arial,sans-serif;letter-spacing:0;
    }
  ...
  const observeRoot=root=>{
    ...
    const label=()=>root.querySelectorAll(selector).forEach(button=>{
      if(button.getAttribute('aria-label')!==(document.documentElement.lang==='en'?'Chat with Latitude Samui':'Discuter avec Latitude Samui'))button.setAttribute('aria-label',(document.documentElement.lang==='en'?'Chat with Latitude Samui':'Discuter avec Latitude Samui'));
    });
